...
namespace myRaiCommonModel.Services
{
    public class SpeseDiProduzioneServizio
    {
        ...
        public IEnumerable<SpeseProduzioneViewModel> GetSpeseProduzione(bool isAperte, DateTime? dataal, DateTime? datadal, string stato, string idFSP)
        {
            decimal saldo = 0;
            ...
            string queryBase =
                        ...
                        " rendicontoVoci.Importo as MA_Importo_In_Euro, " +
                        ...
                        " (rendicontoVoci.Importo-ant.Importo_Totale) as Saldo " +
                        " FROM TImporti_SAP as imp " +
                        " join TFoglio_Spese as spese on imp.ID = spese.ID " +
                        " join TAnticipo as ant on spese.ID = ant.ID " +
                        " join TRendiconto_Voci as rendicontoVoci on ant.ID = rendicontoVoci.ID " +
                        ...
                        " where spese.Matricola like '" + matricola + "' and spese.Data_Creazione >= '2020-01-01' " +
                        " and targ.Tipo <> 'DI' " +
                        " and  tf.NomeTab = '1013' ";
            ...
            if ((string.IsNullOrEmpty(stato) && string.IsNullOrEmpty(idFSP) || dataal.HasValue == false || datadal.HasValue == false))
            {
                if (isAperte)
                {
                    string querySql = queryBase + 
                        ...
                        " and rend.Stato not like 'Q%' and rend.Stato not like '%0' "; 
            ...
            else
            {
                ...
                var searchFSP = from importi in db.TImporti_SAP
                                ...
                                where (importi.MA_Stato == stato && anticipo.Periodo_Dal >= datadal.Value && anticipo.Periodo_Al <= dataal.Value && fogliospese.Matricola == matricola)
        ...
        private string GetProduzioneByMatricola(string term, string desc)
        {
            ...
            if (SessionHelper.Get("_SpeseProd_Desc") != null)
                prodDes = (Dictionary<string, string>)SessionHelper.Get("_SpeseProd_Desc");
            else
            {
                prodDes = new Dictionary<string, string>();
                SessionHelper.Set("_SpeseProd_Desc", prodDes);
            }
        ...
        public IEnumerable<SpeseProduzioneViewModel> GetDettaglioFoglioSpese(decimal id, bool isAperte)
        {
            ...
            string querySql =
                      ...
                      " join TRendiconto_Targhetta as targ on spese.ID = targ.ID " +
                      " join TRendiconto_Voci as rendicontoVoci on spese.ID = rendicontoVoci.ID " +
                      " join TFSP01 as tf on targ.Tipo = tf.Codice " +
                      " join TPROCURATORI as procAnticipo on ant.Procuratore = procAnticipo.CID" +
        ...
        public List<SpeseProduzioneViewModel> GetDescrizioneFromTFSP02ConTipoTaghettaSegreteria(decimal id)
        {
            ...
            var listaDescrizioniAndImporti = descrizioneVociFromTFSP02.Select(s => new SpeseProduzioneViewModel()
                ...
                MA_Importo_In_Euro = s.FirstOrDefault().voci.Importo,
