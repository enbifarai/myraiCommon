...
{
    public class FestivitaHelper
	{
        #region Public

        public static bool IsSemiLavorativo(DateTime date)
        {
            DateTime DataVenerdiSanto = GetPasqua(date.Year).AddDays(-2);
        ...
        public static bool IsFestivo ( DateTime data, string sedeGapp )
		{
			...
			try
			{
				...
				// PATRONO
				//string sedeGapp = Utente.SedeGapp(data);
		...
		/// <param name="data"></param>
		/// <returns></returns>
		public static bool IsExFestivo ( DateTime data )
		{
			bool result = false;
