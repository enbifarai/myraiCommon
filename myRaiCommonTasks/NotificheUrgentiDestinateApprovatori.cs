...
{
	public class NotificheUrgentiDestinateApprovatori
    {
		public NotificheUrgentiDestinateApprovatori ()
		{
		}
		...
		public void SendNotificheESS_LIV1 ( string targetMatricola = null )
		{
			CommonTasks.Log( "\r\n" );
			CommonTasks.Log( "-------------------------------" );
			CommonTasks.Log( String.Format( "Avvio batch di invio notifiche di richieste urgenti {0}\r\n", DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );
			...
			try
			{
                ...
                string oreMinutiRiferimento = dataLimite.Hour.ToString( ).PadLeft( 2 , '0' ) + "00";
                //dataLimite.Minute.ToString().PadLeft(2, '0');

				lNotifiche = GetNotificheESS_LIV1_Urgenti( dataLimite, oreMinutiRiferimento, targetMatricola );
				CommonTasks.Log( String.Format( "Trovate {0} notifiche urgenti - {1}\r\n", lNotifiche.Count(), DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );

				if ( lNotifiche != null )
				{
					InviaUrgentiL1( lNotifiche );
				}				
			}
			catch ( Exception ex )
			{
			...
			CommonTasks.Log( String.Format( "Termine batch di invio notifiche di richieste urgenti {0}\r\n", DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );
		}

		public static List<NotificheMatricola> GetNotificheESS_LIV1_Urgenti ( DateTime dataLimite, string oreMinutiRiferimento, string targetMatricola = null )
		{
			CommonTasks.Log( String.Format( "Reperimento delle notifiche urgenti - {0}\r\n", DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );
			List<NotificheMatricola> notifiche = new List<NotificheMatricola>();

			try
			{
				string tipoNotifica = EnumCategoriaEccezione.MarcaturaUrgente.ToString();
				...
				using ( digiGappEntities db = new digiGappEntities() )
				{
					if ( String.IsNullOrEmpty( targetMatricola ) )
					{
						var ListNotifichePerMatricola = db.MyRai_Notifiche.Where( x => 
							...
							x.descrizione.ToUpper().StartsWith( "NUOVA MARCATURA URGENTE" ) &&
						...
						{
							if ( IsToSendNow( item.matricola, tipoDestinatario.ToString(), "URG", oreMinutiRiferimento ) )
							{
			...
			{
				CommonTasks.Log( String.Format( "Errore nel reperimento delle notifiche urgenti\r\n{0}\r\n{1}\r\n", DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ), ex.ToString() ) );
				notifiche = null;
			}
		...
		public static void InviaUrgentiL1 ( List<NotificheMatricola> lNotifiche )
		{
			...
			foreach ( var item in NotificheDaInviarePerMatr )
			{
				...
				totUrgenti = item.notifiche.Count();
                //destinatario = "RUO.SIP.PRESIDIOOPEN@rai.it";
					...
					using ( digiGappEntities db = new digiGappEntities() )
					{
						item.notifiche.ForEach( w =>
						{
							var toUpdate = db.MyRai_Notifiche.Where( xx => xx.id_riferimento.Equals( w.id_riferimento ) &&
                                                                    xx.descrizione.ToUpper().StartsWith("NUOVA MARCATURA URGENTE")).ToList();
		...
		public static Boolean InviaSingolaMail(string body, string subj, string dest)
        {
            ...
            eml.toList = new string[] { dest };//dest
        ...
        public void SendNotificheUrgentiApprovatoriProduzione ( string targetMatricola = null )
        {
            CommonTasks.Log( "\r\n" );
            CommonTasks.Log( "-------------------------------" );
            CommonTasks.Log( String.Format( "Avvio batch di invio notifiche di richieste urgenti approvatori produzione {0}\r\n" , DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );
            List<NotificheMatricola> lNotifiche = new List<NotificheMatricola>( );
            ...
            try
            {
                ...
                lNotifiche = GetNotificheUrgentiApprovatoriProduzione( dataLimite , oreMinutiRiferimento , targetMatricola );
                CommonTasks.Log( String.Format( "Trovate {0} notifiche urgenti - {1}\r\n" , lNotifiche.Count( ) , DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );
        ...
        public static List<NotificheMatricola> GetNotificheUrgentiApprovatoriProduzione ( DateTime dataLimite , string oreMinutiRiferimento , string targetMatricola = null )
        {
            CommonTasks.Log( String.Format( "Reperimento delle notifiche urgenti approvatori produzione - {0}\r\n" , DateTime.Now.ToString( "dd/MM/yyyy HH:mm:ss" ) ) );
            List<NotificheMatricola> notifiche = new List<NotificheMatricola>( );
