...
namespace myRaiServiceTestClient
{
    ...
    {
        public ProvvedimentiCauseResponse GetProvvedimentiCause()
        {
            ProvvedimentiCauseResponse Response = new ProvvedimentiCauseResponse() { Esito = true, Cause= new List<Causa> (), Provvedimenti =new List<Provvedimento> () };
            ...
            if (RispostaCics.StartsWith("ACK9"))
            {
            ...
            if (iSpazio > 0)
                RispostaCics = RispostaCics.Substring(10) + "".PadRight(iSpazio, ' ');//  Mid(strFunzione, 11) & Space(iSpazio)
            else
                RispostaCics = RispostaCics.Substring(10, LunghezzaRisposta);  // & Mid(strFunzione, 11, CInt(Mid(strACK, 6, 5)))
