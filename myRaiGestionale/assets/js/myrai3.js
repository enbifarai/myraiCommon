        ...
        success: function (data) {
            if (data != null &&
                ...
                if (data.length > 0) {
                    ...
                    var t = $("#divProssimaTrasferta");
                    $(t).html('<div><b>Non ci sono dati da visualizzare</b></div>');
                }
            }
            else {
                var t = $("#divProssimaTrasferta");
...
function continuaSpost(msg) {
    swal({
        title: '<strong>Sposta Direzione</strong>',
        html: "<span><b>"+msg + "</b><br /> Continui o annulli ? </span>",
        showCloseButton: true,
        showCancelButton: true,
        focusConfirm: false,
        confirmButtonText:
            'Continua',
        confirmButtonAriaLabel: 'continua',
        cancelButtonText:
            'Annulla',
...
$(document).ready(function () {
    ...
    $(document.body).on("click", '#jstree .jstree-anchor', function (event) {
        event.stopPropagation();
        var li = $(this).parent("li");
        var idsezione = $(li).attr("data-idsezione");
        debugger
        if (SezioneToMove.id <= 0)
            ShowDettaglio(idsezione);
        else
        {
            if (! ($(li).attr("data-codice").length == 2 || $(li).attr("aria-level") =="2"))
            {
                continuaSpost("Impossibile eseguire questo spostamento");
                return;
            }

            var idSezioneParent = idsezione;
            while (idSezioneParent != undefined)
            {
               
                idSezioneParent = $("li[data-idsezione=" + idSezioneParent + "]").parent("ul").parent("li").attr("data-idsezione");
                console.log(idSezioneParent, SezioneToMove.id);
                if (idSezioneParent != undefined)
                {
                    if (idSezioneParent == SezioneToMove.id)
                    {
                        continuaSpost("Impossibile eseguire questo spostamento");
                        return;
                    }
            ...
            if ($("li[data-idsezione=" + SezioneToMove.id + "]").parent("ul").parent("li").attr("data-idsezione") == idsezione)
            {
                continuaSpost("La relazione è già esistente.");
                return;
            }
            if ( SezioneToMove.id  == idsezione) {
                return;
            }

            swal({
                title: '<strong>Sposta Direzione</strong>',
                html: "<span>Stai spostando <br /><b>" + SezioneToMove.desc + "</b> <br />sotto  <br /><b>" + li.find("a:first").text().trim() + "</b><br /> confermi l'operazione? </span>",
                showCloseButton: true,
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText:
                ...
                moveSezione(SezioneToMove.id, idsezione);
                SezioneToMove.id = 0;
                SezioneToMove.desc = null;
                $("#moving").addClass("hide");
                ShowDettaglio(idsezione);
...
function addResp() {
    ...
    $("#risposte-container i").remove();
    $("#risposte-container").append('<input type="text" class="form-control" style="width:96%;margin-bottom:6px;display:inline" id="risposta_' + (es + 1) + '" name="risposte"/>');
    $("#risposte-container").append('<i class="icons icon-plus" style="cursor:pointer;margin-left:4px" onclick="addResp()"></i>');
}
...
function InitDatePicker() {
    jQuery('.js-datetimepicker').each(function () {
        ...
        $input.datetimepicker({
            ...
            minDate: $input.data('mindata') ? $input.data('mindata') : false,
            maxDate: $input.data('maxdata') ? $input.data('maxdata') : false,
            stepping:$input.data('stepping')?$input.data('stepping'):1,
            icons: {
                time: 'fa fa-clock-o',
                date: 'si si-calendar',
                up: 'fa fa-arrow-up',
                down: 'fa fa-arrow-down',
                previous: 'icons icon-arrow-left-circle',
                next: 'icons icon-arrow-right-circle',
...
function InitDateRangePicker() {
    jQuery('.js-daterangepicker').each(function () {
        ...
        var options = {};
        if ($input.data('parentel'))
            options.parentEl = $input.data('parentEl');

        if ($input.data('startdate'))
            options.startDate = $input.data('startdate');
        if ($input.data('enddate'))
            options.endDate = $input.data('enddate');

        if ($input.data('mindate'))
            options.minDate = $input.data('mindate');
        if ($input.data('maxdate'))
            options.maxDate = $input.data('maxdate');
        
        options.autoApply = true;
        options.showDropdowns = $input.data('showdropdowns') ? $input.data('showdropdowns') : false;
        ...
        if ($input.data('range')) {
            ...
            options.showCustomRangeLabel = $input.data('showdropdowns') ? $input.data('showdropdowns') : true;
...
function AccettaProposte() {
    ...
    $('#table-ecc-auto input[type="checkbox"]:checked').each(function () {
        ...
        {
            idAttivita = $("#att-ceiton-" + $(this).attr("data-cod") + " select").val();
        }
...
function OnDataStrutChange() {
    var dataStruttura = $("#data-strut").val();
    var opened = [];
    $("#tree-container li.jstree-open").each(function () {
        opened.push($(this).attr("data-idsezione"));
    });

    var clicked = $("a.jstree-clicked").parent("li").attr("data-idsezione");
    var clicked_parent = $("a.jstree-clicked").parent("li").parent("ul").parent("li").attr("data-idsezione");

    $.ajax({
        url: "/StrutturaOrganizzativa/getTree",
        type: 'GET',
        ...
        success: function (data) {
            ...
            if (opened.length > 0) {
                for (var i = 0; i < opened.length; i++) {
                    $("#jstree").jstree("open_node", $("[data-idsezione='" + opened[i] + "']"));
                }
            }
            ...
            setTimeout(function () {
                if (clicked) {
                    debugger
                    var a_clicked = $("[data-idsezione='" + clicked + "']>a");
                    if (a_clicked.length > 0) {
                        $("[data-idsezione='" + clicked + "']>a").click();
                        $("#jstree").jstree("select_node", $("[data-idsezione='" + clicked + "']"));
                        $("[data-idsezione='" + clicked + "']>a")[0].scrollIntoView({
                            behavior: "smooth", // or "auto" or "instant"
                            block: "end" // or "end"
                        });
                    }
                    else {
                        $("[data-idsezione='" + clicked_parent + "']>a").click();
                        $("#jstree").jstree("select_node", $("[data-idsezione='" + clicked_parent + "']"));
                        $("[data-idsezione='" + clicked_parent + "']>a").scrollIntoView({
                            behavior: "smooth", // or "auto" or "instant"
                            block: "end" // or "end"
                        });
                    }
...
function ShowDettaglio(idsezione)
{
    var dataStruttura=$("#data-strut").val();

    $.ajax({
        url: "/StrutturaOrganizzativa/getDettaglio",
        type: 'GET',
        data: { idsezione: idsezione, data: dataStruttura, livello: $("#tree-container .jstree-clicked").parents("li").length },
        dataType: "html",
        success: function (data) {
            $("#dettaglio-container").html(data);
            $("#dipendenti-container").html("");
            $("#incarichi-container").html("");
        }
    });
}

function ShowCalendarioTracciati(id, data1,data2,ecc) {
    ...
    $.ajax({
        ...
        dataType: "html",
        data: { idRichiesta: id, d1:data1, d2:data2, ecc:ecc },
        cache: false,
        success: function (data) {
...
function ShowDipendenti(sede,sezione)
{ 
    var dataStruttura = $("#data-strut").val();

    $.ajax({
        url: "/StrutturaOrganizzativa/getDipendenti",
        type: 'GET',
        data: {sede:sede, sezione: sezione, data: dataStruttura },
        dataType: "html",
        success: function (data) {
...
function AddSezione(idsezionePadre)
{
    var dataStruttura = $("#data-strut").val();
    $.ajax({
        url: "/StrutturaOrganizzativa/gestisciSezione",
        type: 'GET',
        cache: false,
        data: { idsezione: idsezionePadre, data: dataStruttura, azione: "N" },
        dataType: "html",
        success: function (data) {
            $("#modal-sezioni-internal").html(data);
            $("#modal-sezioni").modal("show");
        }
    });
}
function GestisciSezione(idsezione, azione)
{
    var dataStruttura = $("#data-strut").val();
    ...
    $.ajax({
        ...
        type: 'GET',
        cache:false,
        data: { idsezione:idsezione , data:dataStruttura, azione:azione},
        dataType: "html",
        success: function (data) {
            $("#modal-sezioni-internal").html(data);
            $("#modal-sezioni").modal("show");
...
function showDettaglioEccezione(codice) {
    ...
    $.ajax({
        url: '/maternitacongedi/getEccezioneSchedaPopup?codice=' + codice,
        type: "GET",
        dataType: "html",
        data: {},
        success: function (data) {
...
function SwalConfirm(title, msg, callback) {
    swal({
        title: title,
        text: msg,
        type: 'warning',
