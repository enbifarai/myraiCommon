...
function FirmaDocEsegui() {
    var ComboEsiste = $("#firma-per-conto").length > 0;
    ...
    var nom = null;
    if (ComboEsiste) {
        pmatr = $("#firma-per-conto").val().trim();
        nom = $("#firma-per-conto option:selected").text().trim();
    }
    ...
    $.ajax({
        ...
        url: "/firma/firma",
        dataType: "json",
        data: { pin: p, pwd: w, pmatr: pmatr, nom: nom },
        cache: false,
        success: function (data) {
...
function RefreshIndex(data1, data2, sede, stato) {
    ...
    $.ajax({
        ...
        error: function (a) { },
        complete: function () { $("#my-block").removeClass("rai-loader"); }
    });

    $("#my-block-pf").addClass("rai-loader");
    ...
    $.ajax({
        ...
        error: function (a) { },
        complete: function () { $("#my-block-pf").removeClass("rai-loader"); }
    });
}
...
function AjaxNotifications(type, mini)
{
    ...
    if (type == 2) //notifiche
    {
        ...
        {
            $("#div-notifiche" + name).html('<div class="rai-popover"><div class="rai-popover-content rai-loader" style="height:50px";></div></div>');
            $.ajax({
                type: 'GET',
    ...
    if (type == 1) //notifiche
    {
        ...
        {
            $("#div-notifiche-1" + name).html('<div class="rai-popover"><div class="rai-popover-content rai-loader" style="height:50px";></div></div>');
            $.ajax({
                type: 'GET',
...
function ResSettimanaleScorri(avanti, tbody, datada, dataa,spanwait)
{
    ...
    $.ajax({
        type: 'GET',
        url: "/resoconti/getResocontiParziale?da=" + datada + "&a=" + dataa +"&avanti="+avanti + "&sede="+tbody,
        dataType: "html",
        data: {},
        cache: false,
        success: function (data) {
...
function openReleaseNoteModale(){
RaiOpenAsyncModal('modal-releaseNotes','/Scrivania/GetNewsReleaseNote');
}
