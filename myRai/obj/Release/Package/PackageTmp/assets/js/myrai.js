...
function Handlers() {
    ...
    $( "#sede,#stato,#eccezione,#nominativo,#datada,#dataal,#livelloDip,#visto" ).on( "change keypress paste input", function ()
    {
        if ( $( "#nominativo" ).val() == "" &&
            $( "#datada" ).val() == "" &&
            $( "#dataal" ).val() == "" &&
            $( "#sede option:selected" ).text() == "Sede" &&
            $("#visto option:selected").text() == "Stato visti" &&
            $( "#stato option:selected" ).text() == "Stato" &&
            $( "#eccezione option:selected" ).text() == "Eccezione" &&
            $("#livelloDip option:selected").val() == "" &&
            !$("#solo-uff-prod").prop('checked'))
        {
    ...
    $("#solo-uff-prod").on("change", function () {
        if ($("#nominativo").val() == "" &&
            $("#datada").val() == "" &&
            $("#dataal").val() == "" &&
            $("#sede option:selected").text() == "Sede" &&
    ...
    $('#form-inserimento').submit(function (e) {
        ...
        var tipidip = $("#giornata-info-attr").attr("data-cambiturno-tipidip");
        var numgiorni = $("#giornata-info-attr").attr("data-cambiturno-numgiorni");
        var tipodip = $("#info-dip").attr("data-tipodip") ;
        var ecc = $("#select-eccezioni").find(":selected").val();

        if (ecc.trim()=="FE" && tipidip.indexOf(tipodip)>=0 && $("#data_da").val() != $("#data_a").val()) {

            var start = moment($("#data_da").val(), "DD/MM/YYYY");
            var end = moment($("#data_a").val(), "DD/MM/YYYY");
            var ndays = moment.duration(end.diff(start)).asDays();
            if (ndays >= numgiorni) {
                $.ajax({
                    url: '/ajax/getInfoSpostamentoRiposi',
                    type: "GET",
                    dataType: "json",

                    data: { datada: $("#data_da").val(), dataa: $("#data_a").val() },
                    success: function (data) {
                        debugger
                        if (data.esito == true) {
                            var msg = "";
                            if (data.giorni.indexOf("-") > 0) {
                                msg = "Vuoi invertire i turni NON LAVORATO/RIPOSO con i giorni <br />" + data.giorni + " ?";
                            }
                            else
                                msg = "Vuoi invertire il turno NON LAVORATO con il giorno <br />" + data.giorni + " ?";

                            swal({
                                title: "Possibilità di inversione codici orario",
                                type: 'question',
                                html: msg,
                                ...
                                $("#cambio-turni").val("1");
                                return InviaInserimentoEccezioni(form);
                            }, function (dismiss) {
                                return InviaInserimentoEccezioni(form);
                            }
                            );
    ...
    function InviaInserimentoEccezioni() {
        debugger
    ...
    $("body").on('change', 'input[type="radio"].cb-ecc-auto', function (evt) {
        
        var ckval = $(this).val();
        $('input[type="radio"].cb-ecc-auto').each(function () {
            if ($(this).val() != ckval) {
                $("#tdnota-" + $(this).val()).hide();
            }
        })
    ...
    $(".content").on('click', '.button-smista', function (evt) {
        OnClick_buttonSmista($(this));
        evt.stopPropagation();
        evt.preventDefault();
        return false;
    });
    ...
    $('#motivo-rifiuto').on('shown.bs.modal', function (event) {
        OnChange_motivodigitato($("#motivo-digitato"));
    });
    $('#motivo-rifiuto').on('show.bs.modal', function (event) {
        $("#eccez-sw").empty();
        $('#alterna option')[0].selected = true;
        $("#div-dalle").hide();
    ...
    $('#giornata-modal').on('show.bs.modal', function (event) {
        ...
        var visto= $(button).closest("tr").find(".button-approva:first").attr("data-richiedevisto") == "1";
        ...
        $.get("/Home/dettagliogiornata?data=" + recipient + "&ideccezionerichiesta=" + IdEccezioneRichiesta, function (data) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (visto) {
                        $("#btnModalApprovaEccezione_" + IdEccezioneRichiesta).attr("data-richiedevisto", "1");
                        $("#DettaglioRifiutaButton").attr("data-richiedevisto", "1");
                    }
                    else {
                        $("#btnModalApprovaEccezione_" + IdEccezioneRichiesta).removeAttr("data-richiedevisto");
                        $("#DettaglioRifiutaButton").removeAttr("data-richiedevisto");
                    }
                },
                error: function (a, b, c) {
                    alert("Failed info " + a + b + c);
...
function refreshDataAsync() {
    var color = "#1d92f5";
    if (document.getElementsByClassName("bg-cda").length) {
        var style = window.getComputedStyle(document.getElementsByClassName("bg-cda")[0], null);
        if (style.length) {
    ...
    $("div[data-async]").each(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('.chart1').easyPieChart({
                    "barColor": color, "delay": 300, scaleColor: false, lineWidth: 8, size: 125, lineCap: 'square'
                });
                $('.chart2').easyPieChart({
                    "barColor": color, "delay": 300, scaleColor: false, lineWidth: 8, size: 125, lineCap: 'square'
                });
...
function OnChange_select_eccezioni(element) {
    ...
    for (var i = 0; i < ControlliPerEccezione.length; i++) {
        if (ControlliPerEccezione[i].codice == codEccez) {
            ...
            if (ControlliPerEccezione[i].richiede_attivita_ceiton && ControlliPerEccezione[i].codice != "CTD")
            {
...
function CaricaPartialExtraParams(partial) {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (partial.toLowerCase()=="cat.cshtml")
                EseguiCat();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result == "OK") {
                ...
                swal({
                    title: 'Attenzione',//anomalie in inserimento
        ...
        if (richiede_ceiton) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#att-ceiton-' + ec + ' select[id="select-richiestaeccezione-idApprovatore-'+ec+'"]').removeAttr('name');
...
function OnChange_notainserimento(element) {
    ...
    $("select.ep-obb:visible").each(function () {
        if ($.trim($(this).val()) == "") EP = false;
    });
...
function OnChange_motivodigitato(textarea) {
    debugger
    if ($("#eccez-sw").is(":visible")) {
        var car = Number($("#eccez-sw option:selected").attr("data-car"));
        if (car == 0)
            $("#conferma-rifiuto").removeAttr("disabled");
        else if ($(textarea).val().trim().length > car) {
            $("#conferma-rifiuto").removeAttr("disabled");
        }
        else {
    ...
    else if ($("#alterna").is(":visible")) {
        var scelta = $("#alterna").val();
        debugger
        if (scelta == "L") {
            $("#conferma-rifiuto").removeAttr("disabled");
        }
...
function OnClick_buttonRifiuta(button) {
    ...
    if (VistoOltreApprovazione(button)) {
        $("#conferma-rifiuto").attr("onclick", "RifiutaEccezione(" + IdEccezioneRichiesta + ",$('#motivo-digitato').val(),this)");
        $("#conferma-rifiuto").attr("data-richiedevisto", "1");
    }
    else {
        $("#conferma-rifiuto").attr("onclick", "RifiutaEccezione(" + IdEccezioneRichiesta + ",$('#motivo-digitato').val())");
        $("#conferma-rifiuto").removeAttr("data-richiedevisto");
    }

    $('#motivo-rifiuto').modal('show');
    $("#motivo-rifiuto #titolo").text("Cancella richiesta");
    $("#motivo-rifiuto #spansw").text("");
}
function VistoOltreApprovazione(element) {
    return $(element).attr("data-richiedevisto") == "1";
}
...
function OnClick_buttonSmista(button) {
    var TRparent = button.closest("tr");
    var IdEccezioneRichiesta = $(TRparent).data("ideccezione");
    SmistaEccezione(IdEccezioneRichiesta, "", false, button);
}
...
function CheckQuantitaDalleAlleSupportoFS() {
   
    var d = $("#param-dalle-orario input").val();
    var a = $("#param-alle-orario input").val();

    $("#button-procedifs").show();
    $("#ecc-p").html("");
    var ab = (/^([0-9]|0[0-9]|1[0-9]|2[0-9]):[0-5][0-9]$/.test(d) && /^([0-9]|0[0-9]|1[0-9]|2[0-9]):[0-5][0-9]$/.test(a));
    if (ab && toMinutes(a) > toMinutes(d)) {
...
function RefreshPartial(url, elements, parentdiv) {
    ...
    fullUrl += 'nome=' + $("#nominativo").val() + '&sede=' + $("#sede").val() + '&stato=' + $("#stato").val() + '&eccezione=' + $("#eccezione").val() + '&data_da='
        + $("#datada").val() + '&data_a=' + $("#dataal").val() + '&titolo=' + $("#titolo").val() + '&luogo=' + $("#luogo").val() + '&visualizzati=' + $("#Visualizzato").val()
        + ($("#visto").val() != undefined ? "&OpzioneRicercaVisti=" + $("#visto").val() :"&OpzioneRicercaVisti=" )
        + '&livelloDip=' + $('#livelloDip').val() + '&solouffprod=' + $('#solo-uff-prod').is(':checked'),
...
function RifiutaEccezione(idRichiestaEccezione, NotaSuRifiuto, buttonProvenienza) {
    debugger
    var url = "/ajax/rifiutaeccezione?id=" + idRichiestaEccezione + "&nota=" + NotaSuRifiuto;
    if (buttonProvenienza != undefined && $(buttonProvenienza).attr("data-richiedevisto") == "1") {
        url += "&visto=true";
    }
...
function ApprovaEccezione(idRichiestaEccezione, NotaSuApprovazione, showConferma, button) {
    var url = "/ajax/validaeccezione?id=" + idRichiestaEccezione + "&nota=" + NotaSuApprovazione + "&visto=" + VistoOltreApprovazione(button);
...
function SmistaEccezione(idRichiestaEccezione) {

    var myHtml = ' <div class="panel-body rai-panel-search"> ' +
        '	<div class="row" id="Titolo">' +
        '		<div class="col-xs-12">' +
        '			<div id="Testo" class=""> <h4 class="base-text rai-font-modal-heading">Seleziona un responsabile a cui smistare la richiesta</h4></div> ' +
        '		</div>' +
        '	</div>' +
        '	<div class="row" id="Contenuto">' +
        '		<div class="col-xs-12">' +
        '			<div id="Corpo">' +
        '               <select id="select-smista-idApprovatore-' + idRichiestaEccezione+'" class="form-control" > '+
        ...
        '	<div class="row">' +
        '		<div class="col-sm-12 ">' +
        '			<div class="rai-btn-fill" id="Azioni">' +
        '				<a href="#" class="swal2-confirm" title="" value=""></a>' +
        '				<a href="#" id="btnSmistaRichiesta" class="btn btn-primary" title="Smista" value="Smista">Smista</a>' +
        '				<a href="#" id="btnAnnullaSmistaRichiesta" class="swal2-cancel btn btn-primary" title="Annulla" value="Annulla">Annulla</a>' +
        '			</div>' +
        '		</div>' +
        '	</div>' +
        '</div>';
        ...
        success: function (data) {
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    daAgg += '<option value="' + data[i].Matricola + '">' + data[i].Nominativo + '</option>';
                }
            }
        },
        error: function (a, b, c) {
    ...
    myHtml = myHtml.replace("##TOREPLACE##", daAgg);
    ...
    $('#btnAnnullaSmistaRichiesta').on('click', function () {
        swal.clickCancel();
        swal.close();
    });

    $('#btnSmistaRichiesta').on('click', function () {
        var questionHtml = ' <div class="panel-body rai-panel-search"> ' +
            '	<div class="row">' +
            '		<div class="col-xs-12">' +
            '			<div id="" class="">'+
            '               <i class="rai-icon fa fa-question" ></i >' +
            '               <h4 class="base-text rai-font-modal-heading">Sicuro di voler smistare la richiesta selezionata?</h4>'+
            '           </div> ' +
            '		</div>' +
            '	</div>' +
            '	<div class="row">' +
            '		<div class="col-sm-12 ">' +
            '			<div class="rai-btn-fill" id="">' +
            '				<a href="#" class="swal2-confirm" title="" value=""></a>' +
            '				<a href="#" id="btnConfermaSmistamento" class="btn btn-primary" title="Smista" value="Conferma">Conferma</a>' +
            '				<a href="#" id="btnAnnullaConfermaSmistamento" class="swal2-cancel btn btn-primary" title="Annulla" value="Annulla">Annulla</a>' +
            '			</div>' +
            '		</div>' +
            '	</div>' +
            '</div>';

        var valoreSelezionato = $('#select-smista-idApprovatore-' + idRichiestaEccezione).val();
        if (valoreSelezionato == "undefined" || valoreSelezionato == "" || valoreSelezionato == null) {
            return false;
        }
        ...
        $('#btnConfermaSmistamento').on('click', function () {
            $.ajax({
                url: "/Ajax/SmistaRichiesta",
                type: "GET",
                data: {
                    idRichiestaEccezione: idRichiestaEccezione,
                    nuovoResponsabile: valoreSelezionato
                },
                success: function (data) {
                    if (data == true) {
                        swal.clickCancel();
                        swal.close();
                        swal("Operazione effettuata con successo", "La richiesta è stata smistata ad altro responsabile", "success");
                        $('#refresh-attivita').click();
                    }
                    else {
                        swal("Operazione non effettuata", "La richiesta non è stata smistata.\nSi è verificato un errore.", "error");
                    }
                },
                error: function (a, b, c) {
                    swal("Operazione non effettuata", "La richiesta non è stata smistata.\nSi è verificato un errore.", "error");
        ...
        $('#btnAnnullaConfermaSmistamento').on('click', function () {
            swal.clickCancel();
            swal.close();
        });
    });
...
function CancellaRichiesta_NonRipianifica(IdRichiestaPadre, isEvidenzaSW,data) {
    ...
    if (isEvidenzaSW) {
        ...
        var tr = $("tr[data-idric-parent='" + IdRichiestaPadre + "']");
        if ($(tr).length > 0) {
            EccRich = $(tr).attr("data-codice-ecc");
            dataLabel = $("#data_da").val();
        }

        var trEsiti = $("tr[data-id-rich='" + IdRichiestaPadre + "']");
        if ($(trEsiti).length > 0) {
            EccRich = $(trEsiti).attr("data-ecc-cod");
            dataLabel = $(trEsiti).attr("data-datarich");
        }

        if (EccRich != null) EccRich = EccRich.trim();
    }
    ...
    $("#spansw").text("Il " + dataLabel + " che lavoro effettuerai?");
    ...
    $("#data-ecc-current").val(dataLabel);
    $('#motivo-rifiuto').modal('show');

    if (isEvidenzaSW) {
        $("#motivo-rifiuto #titolo").text("Richiedi storno per SW del " + dataLabel);
        $("#motivo-rifiuto #spansw").text("Come sostituisci SW del " + dataLabel + "?");
    }
    else {
        $("#motivo-rifiuto #titolo").text("Cancella richiesta");
        $("#motivo-rifiuto #spansw").text("");
...
function CancellaRichiestaSend(IdRichiestaPadre, notaCancellazione, dataRipian) {
    ...
    var eccRimpiazzo = "";
    var eccRimpiazzoDalle = "";
    var richiestoSWH = false;

    if ($("#ripianifica").is(":visible")) {
        if ($("#eccez-sost").is(":visible")) {
            eccRimpiazzo = $("#eccez-sost").val();
        }
        else if ($("#daremoto").is(":visible")) {
            eccRimpiazzo = $("#attiv").val();
        }
    }
    else if ($("#eccez-sw").is(":visible")) {
        eccRimpiazzo = $("#eccez-sw").val();
    }
    if ($("#dalle-storno").is(":visible")) {
        eccRimpiazzoDalle = $("#dalle-storno").val();
    }
    if ($("#div-swh").is(":visible")) {
        richiestoSWH = $("#richiedi-swh").prop("checked");
    }
    debugger
    $.ajax({
        type: 'GET',
        ...
        dataType: "json",
        data: {
            IdRichiestaPadre: IdRichiestaPadre, NotaCancellazione: notaCancellazione, dataRipian: dataRipian, eccRimpiazzo: eccRimpiazzo,
            dalle: eccRimpiazzoDalle, swh: richiestoSWH
        },
        cache: false,
        complete: function () {
        ...
        success: function (data) {
            if (data.result == "OK") {
                ...
                var isVisible = $('#segnalazioniday').is(':visible');
                if (isVisible) RefreshBoxSegnalazioniInPopupRichieste($("#data_da").val());
                swal('OK', 'Cancellazione confermata', 'info').then(function () { if (CiclaEvidenze != null) CiclaEvidenze(); });
                if ($("#dateok").is(":visible")) {
                    $("#dateok").click();
                }
                $("a[data-refreshurl='/home/refreshMieRichieste']").click();
...
function RefreshRiepSett()
{
 var t = $("#dettagliosettimanale");
if (t.length>0)
    {
    $.ajax({
...
function GetProposteAuto(date, forceProp,sessionId) {
   debugger
    ...
    $.ajax({
        ...
        type: "GET",
        dataType: "html",
        data: { date: date1, forceProposte: forceProp, sessionId:sessionId },
        async: true,
        success: function (data) {
            if ($.trim(data) != "") {
                var t = $("#proposteday");
...
function IsDayClosed(date) {
    ...
    if (Disabled_Data_A && Disabled_Data_Da) return false;
...
function ShowWizard2() {
    ...
    if (date1 == '01/01/1900')
    {
        ...
        $(t).html('');
        ShowWizard3();
        $("#modal-popin .block-content-table > .block-content").removeClass("rai-loader");
        return;
    }
    debugger
    //#getecc ////////////////////////////////////////////////////////////////////
    ...
    ///////////////////////////////////////////////////////////////////////////
    $("#modal-popin .modal-content > .block > .block-content").addClass("rai-loader");
    $("#modal-popin .block-content-table > .block-content").addClass("rai-loader");
    $.ajax({
        url: '/ajax/getInfoGiornataAjaxView',
        type: "GET",
        dataType: "html",
        data: { date: date1, sessionId: sessionId },
        success: function (data) {
            var t = $("#infogiornata-content");
            $(t).html(data);
            if (date1 != "01/01/1900" && !IsDayClosed() && !IsDayLockedOrario()) {
                GetProposteAuto(undefined,undefined, sessionId);
            }
            ShowWizard3(sessionId);
            $("#modal-popin .block-content-table > .block-content").removeClass("rai-loader");
            //$("#modal-popin .modal-content > .block > .block-content").removeClass("rai-loader");
        },
        error: function (a, b, c) {
            alert("Failed info " + a + b + c);
        }
...
function ShowWizard3(sessionId) {
    debugger
    var date1 = $("#data_da").val();
    var date = $("#data_a").val();
    ...
    if (!IsDateGreaterThanToday(date1) && date1 != "01/01/1900") {
            ...
            success: function (data) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        var t = $("#segnalazioniday");
                        $(t).html(data);
                        $("#modal-popin .block-content-table").removeClass("rai-loader");
                        UIRai.initblock('#segnalazioniday');
                        ...
                        if (IsDayClosed())
                            ...
                            if (IsDayLockedOrario()) {
                                ...
                                EmptyGiornataChiusa();
                                GetRaggruppamentiDisponibili(sessionId);
                            }
                        }
                    },
                    error: function (a, b, c) {
        ...
        //$("#modal-popin .block-themed > .block-content").addClass("rai-loader");
        $("#modal-popin .modal-content > .block > .block-content").addClass("rai-loader");
        $.ajax({
            url: '/ajax/getSegnalazioniAjaxView',
            type: "GET",
...
var CiclaEvidenze = null;


function AnnullaEvidenze() {
    CiclaEvidenze = null;
}
function ShowPopupGroup(group) {
     
    debugger
    var messaggio = "Sono state rilevate giornate che richiedono un intervento giustificativo prima di poter procedere. Cliccando OK ti verrà mostrata la prima data.";
    $.ajax({
        url: '/ajax/GetTimbratureSW_Evidenze',
        type: "GET",
        dataType: "json",
        data: {},
        success: function (data) {
            if (data.result.length > 0) {
                ...
                swal({
                    ...
                    //CancellaRichiestaDaSegnalazioni(data.result[0].id, true);
                    CiclaEvidenze = function () { ShowPopupGroup(group) };
                    CancellaRichiesta_NonRipianifica(data.result[0].id, true, data.result[0].data)
                })
            }
            else {
                CiclaEvidenze = null;
        ...
        success: function (data) {
            ...
            if (data.result.length > 0) {
                             ...
                             swal({
                                    ...
                                    //CancellaRichiestaDaSegnalazioni(data.result[0].id, true);
                                    CiclaEvidenze = function () { ShowPopupIniziale(msgSeAssenzeIngiustificate, ProvieneDaBoxHome)};
                                    CancellaRichiesta_NonRipianifica(data.result[0].id, true, data.result[0].data) 
                                })                
            }
            else {
        ...
        success: function (data) {
            if (data.result.length > 0) {
                ...
                swal({
                    ...
                    CiclaEvidenze = function () { ShowPopupInizialeGoDate(msgSeAssenzeIngiustificate, ProvieneDaBoxHome, dateToGo) };
                    CancellaRichiesta_NonRipianifica(data.result[0].id, true,data.result[0].data) 
                    //CancellaRichiestaDaSegnalazioni(data.result[0].id, true);
                })
            }
...
function GetRaggruppamentiDisponibili(sessionId) {
    var date1 = $("#data_da").val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (ClickDaSelezione != 0) {
                ClickedTab(1);
                $("#icona-raggr-" + ClickDaSelezione).click();
            }
...
function SelTutti( element, sede )
{
    ...
    $(element).closest("tbody").find(".seltr." + $.trim(sede)).prop("checked", v);
    $(element).closest("tbody").find(".seltr." + $.trim(sede)).change();
}
...
function OnClick_buttonApprovaTutti(button, validazione) {
    ...
    var visto = VistoOltreApprovazione(button);
    if (visto) {
        $("#btn-conf-tutti").attr("data-richiedevisto", "1");
    }
    else {
        $("#btn-conf-tutti").removeAttr("data-richiedevisto");
    }
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if ($(tr).children("td").length < 4)
        {
            $(tr)[0].appendChild(document.createElement("td"));
        }
...
function ApprovaEccezioneDaSerie(idRichiestaEccezione, NotaSuApprovazione, TDinprogress, nextId, indice) {
    ...
    var IsVisto = $("#btn-conf-tutti").attr("data-richiedevisto") == "1";
    var esito = "Validata";

    var url = "/ajax/validaeccezione?id=" + idRichiestaEccezione + "&nota=" + NotaSuApprovazione +"&visto="+IsVisto;
    if (IsValidazione == false) {
        url = "/ajax/rifiutaeccezione?id=" + idRichiestaEccezione + "&nota=" + NotaSuApprovazione + "&visto=" + IsVisto;
        esito = "Rifiutata";
    }
    $.ajax({
        type: 'GET',
...
function GeneralSearch() {
    $("#idEccLoader>.block").addClass("rai-loader");
    ...
    $.ajax({
        ...
        complete: function () {
            $("#idEccLoader>.block").removeClass("rai-loader");
        }
    });
    return false;
}
            ...
            if (data.result == "OK") {
                ...
                if ($(table).find("tr").length == 0) {
                    $(table).removeClass("table")
                        ...
                        .append(
                        '<tr>' +
                            '<td style="padding:15px" class="text-center" colspan="2"><span class="rai-font-md-neutral">Non ci sono dati da visualizzare</span></td>' +
                        '</tr>');
                }
            }
            else {
...
function AbilitaRadio(tipo) {
    ...
    if (tipo == "RR") {
        ...
        debugger
        if (disp >= 1 && SelectContains("RR"))
        {
            $("#gior").removeAttr("disabled");
        }
        if (disp - parseInt(disp) == 0.5) {
...
$(document.body).on("shown.bs.modal", "div[role = dialog]", function (e) {
    ...
    if (!elem.hasClass('js-datetimepicker'))
        elem.focus();
    else {
...
//$(document.body).on("click", ".btn-action-icon", function (event) {
//    addIban(event);
//});
...
function ChooseRangeActivity() {
    ...
    $("#ElencoAttivitaExt").addClass("block rai-loader");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#ElencoAttivitaExt").removeClass("block rai-loader");
        }
    });
}
...
function AttivaAccordion() {
    $('.accordion-sintesi').unbind('click');
...
function AggiungiNotaEccezione()
{
    swal({
        ...
        html:
            ...
            '<br>'+
            '<button type="button" class="btn btn-primary btn-lg" id="btnAggiungiNotaEccezione" >Invia</button>' +
            '<button type="button" class="swal2-confirm btn btn-primary btn-lg" style="display: none;">Invia</button>' +
    ...
    $('#btnAggiungiNotaEccezione').on('click', function () {
        ...
        $.ajax( {
            ...
            success: function ( data )
            {
                ...
                {
                    case "OK":
                        swal( 'Invio messaggio', "Messaggio inviato con successo", "success" );
                        GetNoteRichiesta();
                        break;
                    default:
...
function FiltraSchedeEcc() {
    ...
    $("#schede-eccez").addClass("rai-loader");
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#schede-eccez").removeClass("rai-loader");
...
function showDettaglioEccezione(codice) {
    $("#eccezione-cont").html("");
    $("#eccezione-cont").css("height", "400px");

    $("#eccezione-cont").addClass("rai-loader");
    ...
    $.ajax({
        ...
        data: {},
        success: function (data) {
            $("#eccezione-cont").css("height", "");
            $("#eccezione-cont").removeClass("rai-loader");
...
function CallToSkype(button){
    event.preventDefault();
    var url = $(button).attr('data-skype-url');
