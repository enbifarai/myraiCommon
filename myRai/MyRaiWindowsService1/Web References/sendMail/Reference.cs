...
namespace MyRaiWindowsService1.sendMail
{
    using System;
    ...
    public partial class MailSender : System.Web.Services.Protocols.SoapHttpClientProtocol
    {
        ...
        public MailSender()
        {
            this.Url = global::MyRaiWindowsService1.Properties.Settings.Default.MyRaiWindowsService1_sendMail_MailSender;
        ...
        public void Send(Email objEmail)
        {
			...
			string mail = System.Configuration.ConfigurationManager.AppSettings["DefaultMail"];
			...
			{
				string[] par = Common.GetParametri<string>( EnumParametriSistema.RedirectEmailSuSviluppo );
				if ( par != null && par.Length == 2 && Convert.ToBoolean( par[0] ) == true )
				{
					objEmail.toList = Common.RewriteAddress( MailDestinatario, objEmail.toList, objEmail, par[1].Split( ',' ) );
					objEmail.ccList = Common.RewriteAddress( MailDestinatario, objEmail.ccList, objEmail, par[1].Split( ',' ) );
					objEmail.bccList = Common.RewriteAddress( MailDestinatario, objEmail.bccList, objEmail, par[1].Split( ',' ) );
				}
			}
